...
Master / Slave 双角色异步分区执行控制面，支持 **Cursor** 与 **OpenCode** 两种 agent 运行时。
...
```
.cursor/rules/master-agent.mdc          # Master agent（Cursor 规则，源码）
...
deploy/slave-agent/
  .cursor/rules/slave-agent.mdc         # Slave agent（Cursor 规则，部署源码）
  .opencode/agents/slave-agent.md       # Slave agent（OpenCode，部署源码）
  .cursor/skills/ / .opencode/skills/   # Slave skills（如 memory-monitor）
...
scripts/jobs/
  ...
  slave.conf        # 节点排除 + agent_runtime（auto|cursor|opencode）
...
网关 `slave.conf: agent_runtime` 选择 Cursor CLI、OpenCode CLI 或 `auto`；单次任务可用 `submit.sh --runtime` 覆盖。

## 部署（同步 Master + Slave）

| 脚本 | 目标 | 安装内容 |
|------|------|----------|
| `deploy-master.sh [HOST\|local]` | Master（默认本工作区） | Master agent 规则 + OpenCode agent + `opencode.json`，`submit.sh` / `poll.sh` / `master.conf` |
| `deploy-slave.sh <gateway>` | Slave 网关（如 `cn1`） | Slave agent 规则 + OpenCode agents/skills + `opencode.json`，`run-slave.sh` / `slave.conf`，`/var/agent-jobs/` |
...
- `/home/code/agents/.cursor/skills/`（Slave 侧 tool skills，如 `memory-monitor`）
- `/var/agent-jobs/` 任务目录
...
| Agent | `--prompt '<task>'` | **Slave agent LLM**（网关 Cursor CLI 或 OpenCode CLI）；运行时由 `slave.conf: agent_runtime` 决定（`auto\|cursor\|opencode`，可用 `--runtime` 覆盖） |
...
| 角色 | Cursor（`.mdc`） | OpenCode | 中文文档 |
|------|------------------|----------|----------|
| Master | `.cursor/rules/master-agent.mdc` | `.opencode/agents/master-agent.md` | `docs/zh/master-agent.md` |
| Slave | `~/.cursor/rules/slave-agent.mdc`（cn1） | `.opencode/agents/slave-agent.md` | `docs/zh/slave-agent.md` |
| Skill：内存监控 | `deploy/slave-agent/.cursor/skills/memory-monitor/`（**仅 Slave 网关**） | 同路径 `.opencode/skills/` | `SKILL.zh.md` / `docs/zh/memory-monitor.md` |
| Skill：add-tools2 | `.cursor/skills/add-tools2/`（**仅 Master**） | `.opencode/skills/add-tools2/` | `SKILL.zh.md` |

运行时加载 Cursor 规则或 OpenCode agent 定义；中文文档供团队阅读与运维参考。
