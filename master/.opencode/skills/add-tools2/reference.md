...
Reference for `/add-tools2` (Master-only meta skill). Example deployed skills live under `slave/.opencode/skills/`.
...
| Meta skill (OpenCode) | `.opencode/skills/add-tools2/` | Master OpenCode; never under `slave/` |
...
|-------|-------------|---------------------------|
| **slave** | `slave/.opencode/skills/<name>/` (+ OpenCode) | Master: `submit.sh --prompt` delegate; Slave: gateway commands |
...
- Directory: `slave/.opencode/skills/{{SKILL_NAME}}/`
- OpenCode mirror: `slave/.opencode/skills/{{SKILL_NAME}}/`
...
When the new skill is user-facing on Slave, add a row to `slave/.opencode/agents/slave-agent.md`:
