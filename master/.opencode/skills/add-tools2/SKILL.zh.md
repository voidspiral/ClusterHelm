...
| **生成的 tool skill（slave / both）** | `slave/.opencode/skills/<name>/` + `.opencode/skills/<name>/` | `deploy-slave.sh` |
...
| 参数 | 示例 | 说明 |
|------|------|------|
| **工具路径** | ``scripts/monitor`、`master/scripts`、`slave/scripts` | 相对仓库根目录，或绝对路径 |
...
4. **对照** `slave/.opencode/skills/` 下已有 skill（结构与语气参考，非硬套某一工具）。
...
生成的 **tool skill** 不要与 **meta skill**（`add-tools2`）混放。**不要** 将 `add-tools2` 放入 `slave/`。
...
- [ ] 若需 Slave 路由：更新 `slave/.opencode/agents/slave-agent.md` skills 表
- [ ] 若 scope 含 slave：提示 `./scripts/deploy/deploy-slave.sh <gateway>`（不部署 `add-tools2`）
