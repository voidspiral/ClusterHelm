...
[[ -f "$ROOT/master/.opencode/agents/master-agent.md" ]] && pass "master-agent.md exists" || fail "missing master-agent.md"
...
MASTER_PROMPT='向 test 分区执行 hostname -s。必须使用 ./scripts/submit.sh --partition test 提交，然后用 ./scripts/poll-wait.sh 阻塞等待完成，最后呈现 partition_report.markdown。禁止直接 ssh 到 cn1-cn10 执行命令。'

# --- 3.2 Master OpenCode ---
run_section "Master OpenCode → test partition"
OC_LOG="$LOG_DIR/master-opencode.log"
if (cd "$ROOT/master" && opencode run --agent master-agent --auto "$MASTER_PROMPT" > "$OC_LOG" 2>&1); then
