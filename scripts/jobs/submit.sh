...
usage() {
  echo "Usage: $0 --partition EXPR --command CMD [--task TITLE] [--gateway HOST] [--deadline SEC]" >&2
...
out=$(ssh -o ConnectTimeout="$(read_master_default ssh_connect_timeout 15)" -o BatchMode=yes "$GATEWAY" \
  "timeout $SUBMIT_TIMEOUT bash '$REMOTE_SCRIPT' submit \
    --partition $(printf %q "$PARTITION") \
    --command $(printf %q "$COMMAND") \
    $( [[ -n "${TASK_TITLE:-}" ]] && printf -- '--task %q' "$TASK_TITLE" ) \
    --deadline $DEADLINE_SEC" 2>&1) || {
