...
def load_slave_conf(script_dir: Path | None = None) -> dict[str, Any]:
    # Prefer slave/config/slave.conf (repo + deployed layout).
    here = Path(__file__).resolve().parent
    script_dir = script_dir or here
    candidates = [
        script_dir.parent.parent / "config" / "slave.conf",  # .../scripts/preflight → slave/config
        script_dir.parent / "config" / "slave.conf",
        script_dir / "slave.conf",
    ]
    conf_path = next((p for p in candidates if p.is_file()), candidates[0])
    out: dict[str, Any] = {
