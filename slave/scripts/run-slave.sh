...
slave_conf_get() {
  ...
  conf="$(cd "$(dirname "$0")/../config" && pwd)/slave.conf"
  if [[ -f "$conf" ]]; then
    local v
...
job_dir, job_id, ssh_timeout, script_dir, preflight_dir = (
...
def cleanup_mpi_hosts(hosts):
    # Repo: <root>/scripts/mpi; gateway may mirror under scripts/mpi next to run-slave.
    candidates = [
        Path(script_dir) / "mpi" / "cleanup-mpi.sh",
        Path(script_dir).parents[1] / "scripts" / "mpi" / "cleanup-mpi.sh",
    ]
    script = next((p for p in candidates if p.is_file()), candidates[0])
